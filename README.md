...
- 智能分析工作台：多模型并行 AI 研判（OpenAI 兼容接口）、网络研究简报本地留存、随机因子（全球市场/国内市场/政策面等 tag 可勾选）、共识买入/卖出/持有/观望信号
...
（DeepSeek、通义、Moonshot、OpenAI 等，密钥仅保存在本机）与可选的 Tavily
网络研究密钥；勾选随机因子（全球市场、国内市场、政策面等）。智能分析页可
手动触发，或在交易时段（默认 09:00–15:00，可配间隔）自动对“参与智能分析”
的标的执行扫描：每次分析并行调用所有启用模型 → 共识信号（买入/卖出/持有/
观望）→ 会话留存（含网络简报、因子种子、各模型结论），并可按时间维度回看、
作为同标的后续分析的记忆参考。模型调用与搜索经 Rust 命令
（`llm_chat_completion` / `web_search_tavily`）执行，密钥随调用传递、不落盘。
